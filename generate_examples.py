...
layer_sizes = [1,4,5,4,1]     # layer sizes in neural network, first must be 1, last must be 1, middle layers can change size or add more
activations = ['tanh','tanh','tanh','linear']  # NN activations, list must be 1 less in length than above layer sizes.  Stick to pattern of tanh, tanh, ... tanh, sigmoid
N_training = 40                # number of training steps for the NN fit.
...
input_features, input_vectors, input_trees, losses = generate_training_examples(N_training,
                                    ...
                                    N_epochs=N_epochs,learning_rate=learning_rate,verbose=False,uniquify=False,lambda_reg=.1)
...
pickle.dump(allowed, open( "data/allowed.p", "wb" ) )
